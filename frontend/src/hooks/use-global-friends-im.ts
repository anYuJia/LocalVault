...
async function maybeAutoReply(senderUid: string, incoming: LocalChatMessage, recentMessages: LocalChatMessage[], repliedKeys: Set<string>) {
  ...
  try {
    ...
    if (!shouldAutomateText(incomingText, config, "private")) {
      logger.addLog("好友私信未触发自动回复：未命中过滤规则", "info");
      return;
    }
    ...
    logger.addLog(`好友私信自动回复成功：${displayName}`, "success");
  } catch (error) {
